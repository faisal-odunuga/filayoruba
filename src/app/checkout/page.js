...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    ...
    const saved = sessionStorage.getItem("fila_checkout_prefill");
    if (saved) {
      try {
  ...
  const handleChange = (e) => {
    ...
    setForm((prev) => {
      ...
      sessionStorage.setItem("fila_checkout_prefill", JSON.stringify(next));
      return next;
    });
  };
    ...
    return (
      <main className="pt-32 pb-20 px-6 md:px-12 max-w-4xl mx-auto text-center space-y-6">
        ...
        <div className="flex justify-center gap-4 mt-6">
          <Link href="/filas" className="bg-primary text-black px-8 py-4 font-headline font-bold uppercase tracking-widest">
            Browse Filas
          </Link>
              ...
              {cart.map((item) => (
                <div key={`${item.id}-${item.size}`} className="flex gap-4 border-b border-white/10 pb-4">
                  <div className="w-16 h-20 bg-surface overflow-hidden shrink-0 relative">
                    <Image src={item.image} alt={item.category} fill sizes="64px" className="object-cover" />
                  </div>
                  <div className="flex-1">
                    <p className="font-headline font-bold uppercase leading-tight">{item.category}</p>
