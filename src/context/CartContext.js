...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    try {
      const storedCart = typeof window !== 'undefined' ? localStorage.getItem('fila_cart') : null;
      const storedLastOrder = typeof window !== 'undefined' ? localStorage.getItem('fila_last_order') : null;
      const storedOrders = typeof window !== 'undefined' ? localStorage.getItem('fila_orders') : null;
  ...
  useEffect(() => {
    try {
      localStorage.setItem('fila_cart', JSON.stringify(cart));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('fila_last_order', JSON.stringify(lastOrder));
    } catch (err) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('fila_orders', JSON.stringify(orders));
    } catch (err) {
